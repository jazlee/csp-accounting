...
opj = os.path.join
ad = tools.config['ecfserver.api.path']
sys.path.insert(1,ad)

def registerClasses():
  _mod_list = []
  module_list = os.listdir(ad)
  pool = ecfpool.getAPIPool()
  ...
  for mod in _mod_list:
    # logger.notifyChannel('init', ecflogger.LOG_INFO, 'API:%s:registering business processor class' % mod)
    # sys.stdout.flush()
    try:
      imp.load_module(mod, *imp.find_module(mod))
      pool.instanciate(mod)
    except Exception, e:
      print >>sys.stderr, 'ERROR on loading API module %s, caught exception: %s' % (mod, e)
