...
def registerClasses():
  ...
  ad = tools.config['ecfserver.models.path']
  _mod_list = []
  ...
  for _mod in _mod_list:
    ...
    try:
      try:
        imp.load_module('tbl', fp, pathname, description)
        # logger.notifyChannel('init', ecflogger.LOG_INFO, 'TBL:%s:registering model class' % _mod)
      except Exception, e:
        print >>sys.stderr, 'ERROR on loading table %s, caught exception: %s' % (_mod, e)
