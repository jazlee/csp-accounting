...
class MemCached(object):
  ...
  def get(self, key, default=None):
      try:
        base64val = _ecfpyutils.memcached_get("%s:%s" % (self.namespace, key))
        streamval = base64.decodestring(base64val)
        retvalue = cPickle.loads(streamval)
      except:
        return default
      return retvalue

  def put(self, key, val):
      streamval = cPickle.dumps(val)
      base64val = base64.encodestring(streamval)
      _ecfpyutils.memcached_put("%s:%s" % (self.namespace, key), base64val)

  def delete(self, key):
    _ecfpyutils.memcached_del("%s:%s" % (self.namespace, key))
