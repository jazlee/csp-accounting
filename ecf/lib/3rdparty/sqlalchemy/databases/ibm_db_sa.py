...
if sqlalchemy.__version__ > 0.5:
    # SA 0.5.x
    from sqlalchemy import log as logging
...
from sqlalchemy import sql, engine, schema
...
class IBM_DBDialect(default.DefaultDialect):
  ...
  def create_connect_args(self, url):
    ...
    if str(url).find('///') != -1:
      ...
      dsn = ';'.join(dsn_param)
      dsn += ';'
      dialect.logger.debug("\n  ***  IBM_DBDialect::create_connect_args: " + repr(dsn))
  ...
  def create_execution_context(self,
    ...
    """
    dialect.logger.debug("\n  ***  IBM_DBDialect::create_execution_context: " + repr(statement))
  ...
  def has_table(self, connection, table_name, schema=None):
    ...
    dialect.logger.debug("\n  ***  IBM_DBDialect::has_table( "+str(table_name)+', '+repr(schema)+' ) = '+str(has_it))
    return has_it
  ...
  def reflecttable(self, connection, table, include_columns = None):
    ...
    """
    dialect.logger.debug("\n  ***  IBM_DBDialect::reflecttable( "+repr(table)+', '+str(include_columns)+' )')
