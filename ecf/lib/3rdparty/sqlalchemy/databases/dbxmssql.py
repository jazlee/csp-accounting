...
"""MSSQL backend, thru either pydbxapi, pymssql, adodbapi or pyodbc interfaces.
...
class MSSQLDialect(default.DefaultDialect):
    ...
    def dbapi(cls, module_name=None):
        if module_name:
            ...
            for dialect_cls in [MSSQLDialect_pyodbc, MSSQLDialect_pydbxapi, MSSQLDialect_pymssql, MSSQLDialect_adodbapi]:
                try:
                    return dialect_cls.import_dbapi()
...
class MSSQLDialect_pydbxapi(MSSQLDialect):
    supports_sane_rowcount = False
    max_identifier_length = 30

    def import_dbapi(cls):
        import pydbxapi as module
        return module
    import_dbapi = classmethod(import_dbapi)
    ...
    def __init__(self, **params):
        super(MSSQLDialect_pydbxapi, self).__init__(**params)
        self.use_scope_identity = True
    ...
    def create_connect_args(self, url):
        r = super(MSSQLDialect_pydbxapi, self).create_connect_args(url)
        return r
    ...
    def make_connect_string(self, keys):
        ...
        if 'port' in keys:
            connectors.append ("HostName=%s:%s" % (keys.get("host"), keys.get("port")))
        else:
            connectors.append ("HostName=%s" % keys.get("host"))
        connectors.append ("DataBase=%s" % keys.get("database"))
        user = keys.get("user")
        if user:
            connectors.append("User_Name=%s" % user)
            connectors.append("Password=%s" % keys.get("password", ""))
        return [[], {'dsn':";".join (connectors), 'drv':'', 'getdrvfnc':'', 'libnm':'', 'vlibnm':''}]
...
dialect_mapping = {
    'pydbxapi': MSSQLDialect_pydbxapi,
