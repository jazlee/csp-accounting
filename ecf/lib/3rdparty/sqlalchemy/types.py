...
class AbstractType(object):
    ...
    #this is ECF Extension
    def get_ecf_type(self, dbapi):
        """Return the corresponding type object from the underlying fake DB-API.

        This tweak is need by ECF BE-Server to bind compatible field type
        """
        return None

    def get_ecf_type_ex(self):
        import fakeapi
        return self.get_ecf_type(fakeapi)
...
class TypeDecorator(AbstractType):
    ...
    def get_ecf_type(self, dbapi):
        return self.impl.get_ecf_type(dbapi)
...
class Currency(Numeric):
    def __init__(self, precision=4, length=18, asdecimal=True):
        self.precision = precision
        self.length = length
        self.asdecimal = asdecimal
